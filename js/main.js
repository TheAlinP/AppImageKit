/* JSLint options */
/*jslint browser:true, devel:true, multivar:true, for:true*/
/*global window, $, TweenMax, Bounce, windowWidth, AppDir, AppDirWidth,
 leftOffset, AppDirHeight, imageWidth, initialAnimationsReady, TimelineMax,
 Power2, startButton*/
...
function startAnimation() {
    ...
    function positionNewApp() {
        ...
        // calculate an offset
        leftOffset = ((document.documentElement.clientWidth) - imageWidth) / 2;
        // set an offset
    ...
    function resizeAppDir(param) {
        ...
        if (param === "init") {
            // put the AppDir <div> in a global variable
        ...
        if (windowWidth <= 299) {
            ...
            // if the browser window is 300-1000 pixels set a variable width
            window.AppDirWidth = 300 + ((windowWidth - 300) / 3);
        } else {
            // otherwise set a fixed width
            window.AppDirWidth = 300 + ((1001 - 300) / 3);
        }
        // update the width
        AppDir.style.width = AppDirWidth + "px";
        // calculate a left offset
        window.leftOffset = (windowWidth - $(AppDir).outerWidth()) / 2;
        ...
        window.AppDirHeight = AppDirWidth * 0.50;
        // add a height
        ...
        window.imageWidth = AppDirWidth * 200 / (300 + ((1001 - 300) / 3));
        // select the icons
        ...
        // run it each time the window is resized
        ...
        (function () {
            ...
            TweenMax.to(progressStatus, 5, {var: 100,
                ...
                document.getElementById("progress-status").setAttribute(
                    "data-value",
                    Math.ceil(progressStatus.var)
                );
                // do the same with the fallback progress bar's inner text
                document.querySelector("#fallback-progress-bar span").innerHTML =
                        Math.ceil(progressStatus.var) + "%";
            }});
        }());
    ...
    function createFileIcons(param) {
        ...
        function checkType(v) {
            var variableTypes, ToString;
            // create an object with variable types
            variableTypes = {
                "undefined": "undefined",
                "number": "number",
                "boolean": "boolean",
            ...
            return variableTypes[typeof v] || variableTypes[ToString.call(v)] || (v
                ? "object"
                : "null");
        }
    ...
    // create <img> elements with the file icons URLs
